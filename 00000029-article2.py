...
# Function to download article content with error handling
def download_article(url, retries=3, backoff_factor=0.3):
    for attempt in range(retries):
        try:
            ...
            if response.status_code == 403:  # Forbidden access
                print(f"Access forbidden for URL {url}: {e}")
                break
            print(f"HTTP error on attempt {attempt + 1} for URL {url}: {e}")
        except requests.exceptions.RequestException as e:
            print(f"Request error on attempt {attempt + 1} for URL {url}: {e}")
        except Exception as e:
            print(f"Error on attempt {attempt + 1} for URL {url}: {e}")
        time.sleep(backoff_factor * (2 ** attempt))  # Exponential backoff

    raise Exception(f"Failed to download article from {url} after {retries} attempts.")
...
def fetch_articles(category, num_articles):
    ...
    for api_url, api_key in [(NEWS_API_URL, NEWS_API_KEY), (CURRENTS_API_URL, CURRENTS_API_KEY)]:
        try:
            ...
            print(f"Request exception fetching articles from {api_url}: {e}")
        if len(articles) >= num_articles:
            break
    return articles[:num_articles]

# Function to process articles and save summaries
def process_articles(category, num_articles):
    ...
    for i, article in enumerate(tqdm(articles, desc=f"Summarizing {category} articles"), start=1):
        try:
            article_text = download_article(article['url'])
            summary = summarize_pegasus(article_text)
            output_path = os.path.join(output_dir, f"{category}{i}.json")
            with open(output_path, 'w') as file:
                json.dump({'summary': summary, 'original_text': article_text}, file, indent=4)
        except Exception as e:
            print(f"Error summarizing article {i} for category '{category}': {e}")
